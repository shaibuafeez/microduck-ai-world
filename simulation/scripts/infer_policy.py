...
def main():
    ...
    parser.add_argument("--speak", action="store_true",
                        help="Say the duck's thoughts out loud. Off by default — the "
                             "narration gets old fast; the text lines are still printed.")
    parser.add_argument("--race", action="store_true",
                        help="Race mode: lap timing over scene_race.xml. Use with the roller "
                             "policy and --scene .../scene_race.xml — rollers are trained to "
                             "0.6 m/s where walking tops out at 0.4.")
    parser.add_argument("--blind", action="store_true",
                        help="Run --brain without the head camera (text events only).")
    parser.add_argument("--brain", action="store_true",
                        help="Drive velocity commands from the 0G brain (scripts/duck_brain.py). "
                             "Set OG_API_KEY to go online; without it the duck falls back to a "
                             "local wander, which is also what happens when the network drops.")
    parser.add_argument("--scene", type=str, default=None,
                        help="Path to a scene XML, overriding the default/roller/ball choice "
                             "(e.g. src/mjlab_microduck/robot/microduck/scene_obstacles.xml)")
    ...
    # An explicit --scene wins over all of the above.
    if args.scene:
        xml_path = args.scene
    ...
    # --- 0G brain ---------------------------------------------------------
    # Owns nothing: it writes velocity commands, exactly as the keyboard does.
    # Nothing about balance, footsteps or the policy stack is aware of it.
    brain = brain_sensor = None
    duck_ears = None
    duck_text = None
    duck_race = None
    _last_race_msg = ""
    duck_brain_logfile = None
    if args.brain:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from duck_brain import (DuckBrain, Ears, Eyes, SimSensor, TextChannel,
                                speak as duck_brain_speak)
        brain = DuckBrain()
        brain_sensor = SimSensor(model, data, policy.trunk_base_id)
        if not args.blind:
            eyes = Eyes(model, data, policy.trunk_base_id)
            brain.eyes = eyes if eyes.ok else None
            print(f"Eyes: {'ON' if eyes.ok else 'OFF — ' + eyes.error}")
        ears = Ears()
        duck_ears = ears
        say_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                "..", "say_to_duck.txt")
        duck_text = TextChannel(say_path)
        print(f'Talk to it:  echo "kick the ball into the goal" >> {os.path.abspath(say_path)}')
        print(f"Ears: {'ON  press V to talk (' + ears.backend + ')' if ears.ok else 'OFF (needs ffmpeg)'}")
        brain_log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      "..", "brain.log")
        duck_brain_logfile = open(brain_log_path, "w", buffering=1)
        print(f"Brain log: {os.path.abspath(brain_log_path)}")
        if args.race:
            from duck_brain import Race
            cp_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..",
                                   "src/mjlab_microduck/robot/microduck/race_checkpoints.txt")
            cps = [tuple(float(v) for v in ln.split())
                   for ln in open(cp_file) if ln.strip()]
            brain.race = Race(cps)
            duck_race = brain.race
            print(f"Race: {len(cps)} gates, lap timing on")
        brain.log("spawned in the yard")
        brain.start()
        print(f"Brain: {'ONLINE' if brain.online else 'OFFLINE (no OG_API_KEY — local wander)'}"
              + (f", model {os.environ.get('OG_BRAIN_MODEL', 'qwen3-vl-30b')}"
                 if brain.online else ""))
    ...
    if args.race:
        # The spawn above is hardcoded to the origin, which in scene_race.xml is
        # the middle of the oval — the duck would never reach gate 1. Put it on
        # the start line, facing along the track (+Y at gate 0).
        data.qpos[qpos_adr + 0] = 2.60
        data.qpos[qpos_adr + 1] = -0.35
        data.qpos[qpos_adr + 3:qpos_adr + 7] = [0.7071, 0, 0, 0.7071]
    ...
    def handle_key(key):
        ...
        try:
            if key == "up":
                ...
                # Push-to-talk ("v" for voice — "t" was already taken by the
                # policy-inference toggle). Recording runs on its own thread so
                # the control loop never stalls on the microphone.
                if duck_ears is not None and duck_ears.ok:
                    print(f"[listening {duck_ears.SECONDS}s...]")
                    duck_ears.listen_async(brain)
                else:
                    print("[ears off — needs ffmpeg and OG_API_KEY]")
            elif key == "k":
    ...
    with TerminalInput() as term, \
        ...
        try:
            ...
            while viewer.is_running() and not quit_requested:
                ...
                # --- 0G brain ------------------------------------------------
                # Read whatever intent is currently held and act on it. NEVER
                # wait: the brain thread refreshes on its own clock, so a stale
                # intent is the normal case, not an error condition.
                if brain is not None:
                    brain_sensor.poll(step_start, brain)
                    if brain_sensor.needs_reset:
                        brain_sensor.needs_reset = False
                        _kf = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_KEY, "STAND")
                        _xy = data.qpos[policy._trunk_qpos_adr:policy._trunk_qpos_adr + 2].copy()
                        mujoco.mj_resetDataKeyframe(model, data, _kf)
                        data.qpos[policy._trunk_qpos_adr:policy._trunk_qpos_adr + 2] = _xy
                        mujoco.mj_forward(model, data)
                        print("[reset] stood back up (no stand-up policy available)")
                    if duck_race is not None:
                        _p = data.xpos[policy.trunk_base_id]
                        _m = duck_race.update(float(_p[0]), float(_p[1]), step_start)
                        if _m and _m != _last_race_msg:
                            _last_race_msg = _m
                            print(f"[race] {_m}")
                            brain.log(f"RACE: {_m}")
                    if duck_text is not None:
                        heard = duck_text.poll(brain)
                        if heard:
                            print(f'[told] "{heard}"')
                    intent = brain.current()
                    policy.set_vel_cmd(intent.vx, 0.0, intent.wz)

                    # Act on each intent ONCE. The brain refreshes on its own
                    # clock and the loop runs 100x faster, so without this the
                    # duck would retrigger the same trick every tick.
                    if intent.stamp != brain_sensor.acted_stamp:
                        brain_sensor.acted_stamp = intent.stamp
                        if intent.say and args.speak:
                            duck_brain_speak(intent.say)
                        if intent.action != "none":
                            # every trigger guards its own preconditions and
                            # just prints if the state machine is busy
                            if intent.action == "ground_pick":
                                policy.trigger_ground_pick()
                            elif intent.action == "sit":
                                policy.toggle_sit()
                            else:
                                policy.trigger_behavior(intent.action)
                        lat = f" {brain.latency*1000:.0f}ms" if brain.calls else ""
                        eye = "*" if brain.frames else ""
                        if intent.target:
                            st = ("ARRIVED" if intent.arrived
                                  else "sighted" if intent.sighted else "searching")
                            line = (f"[seek{eye}:{intent.source}{lat}] {intent.target} — {st}"
                                    f"  bearing {intent.bearing:+.0f}deg"
                                    + (f'  "{intent.say}"' if intent.say else ""))
                        else:
                            line = (f"[brain{eye}:{intent.source}{lat}] {intent.mood}"
                              + (f" -> {intent.action}" if intent.action != "none" else "")
                              + (f' "{intent.say}"' if intent.say else "")
                              + f"  (vx={intent.vx:+.2f} wz={intent.wz:+.2f}"
                              + (f", {brain.errors} err" if brain.errors else "") + ")")
                        print(line)
                        if duck_brain_logfile is not None:
                            duck_brain_logfile.write(line + "\n")
